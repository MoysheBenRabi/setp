#ifndef MXP_MESSAGE_INTERACT_RSP
#define MXP_MESSAGE_INTERACT_RSP
 ...
 */
class interact_rsp : public message {
public:

    /*! Interaction responce */ 
    response_fragment    response_header;
    /*! Resulting description of the interaction */
    interaction_fragment response;

    /*! Constructor. */
    interact_rsp() : message(INTERACT_RSP) {}

    /*! Virtual destructor. */
    virtual ~interact_rsp() {}

    /*! Copy constructor.

        \param irt_rsp the object to base this copy on.
     */
    interact_rsp(const interact_rsp & irt_rsp) : message(INTERACT_RSP),
        response_header(irt_rsp.response_header),
        response(irt_rsp.response) {
    }

    /*! Assignment operator.

        \param irt_rsp the object to base this object on.
        \return a reference to this object, after the assignemt has been made.
     */
    interact_rsp& operator=(const interact_rsp & irt_rsp) {
        if (this == &irt_rsp) {
            return *this;
        }
        response_header = irt_rsp.response_header;
    ...
    virtual bool equals(const message & other) const {
        return (this == &other) ||
              ((get_type() == other.get_type()) &&
               (response_header == ((interact_rsp &) other).response_header) &&
               (response == ((interact_rsp &) other).response));
    }

    virtual unsigned int size() const {
        return response_header.size() + response.size();
    }
    ...
    serialize(serialization::byte_writer_base & writer) const {
        ...
        counter += response.serialize(writer);
        return counter;
    }
    ...
    deserialize(serialization::byte_reader_base & reader, unsigned int len) {
        ...
        response.deserialize(reader,len);
    }
...
/*! Write a human-readable representation of a message to an output stream.
    ...
    \param irt_rsp the message to write.
    \return the output stream after the message has been written to it.
*/
inline
std::ostream& operator<<(std::ostream &os, const interact_rsp & irt_rsp) {
    os << "interact_rsp[type: " << irt_rsp.get_type();
    os << ",response_header=";
    os << irt_rsp.response_header;
    os << ",response=";
    os << irt_rsp.response;
    os << "]";
    return os;
