#ifndef MXP_MESSAGE_DETACH_RSP
#define MXP_MESSAGE_DETACH_RSP
 ...
 */
class detach_rsp : public message {
public:

    /*! The detach response fragment */
    response_fragment response_header;

    /*! Constructor. */
    detach_rsp() : message(DETACH_RSP) {}
    ...
    /*! Copy constructor.
        ...
        \param dch_rsp the object to base this copy on.

    */
    detach_rsp(const detach_rsp & dch_rsp) : message(DETACH_RSP) {
        response_header = dch_rsp.response_header;
    }

    /*! Assignment operator.

        \param dch_rsp the object to base this object on.
        \return a reference to this object, after the assignemt has been made.

    */
    detach_rsp& operator=(const detach_rsp & dch_rsp) {
        if (this == &dch_rsp) {
            return *this;
        }
        response_header = dch_rsp.response_header;
    ...
    virtual bool equals(const message & other) const {
        return (this == &other) ||
               ((get_type() == other.get_type()) &&
                (response_header == ((detach_rsp &) other).response_header));
    }
...
/*! Write a human-readable representation of a message to an output stream.
    ...
    \param dch_rsp the message to write.
    \return the output stream after the message has been written to it.
*/
inline
std::ostream& operator<<(std::ostream &os, const detach_rsp & dch_rsp) {
    os << "detach_rsp[type: " << dch_rsp.get_type();
    os << ",response_header=";
    os << dch_rsp.response_header;
    os << "]";
    return os;
}
