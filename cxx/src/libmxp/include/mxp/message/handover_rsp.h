#ifndef MXP_MESSAGE_HANDOVER_RSP
#define MXP_MESSAGE_HANDOVER_RSP
...
/*! A Detach response message, as specified by MXP.
 */
class handover_rsp : public message {
public:
    ...
    /*! Constructor. */
    handover_rsp() : message(HANDOVER_RSP) {}

    /*! Virtual destructor. */
    virtual ~handover_rsp() {}

    /*! Copy constructor.

        \param hovr_rsp the object to base this copy on.

    */
    handover_rsp(const handover_rsp & hovr_rsp) : message(HANDOVER_RSP) {
        response_header = hovr_rsp.response_header;
    }

    /*! Assignment operator.

        \param hovr_rsp the object to base this object on.
        \return a reference to this object, after the assignemt has been made.

    */
    handover_rsp& operator=(const handover_rsp & hovr_rsp) {
        if (this == &hovr_rsp) {
            return *this;
        }
        response_header = hovr_rsp.response_header;
    ...
    virtual bool equals(const message & other) const {
        return (this == &other) ||
               ((get_type() == other.get_type()) &&
                (response_header == ((handover_rsp &) other).response_header));
    }
...
/*! Write a human-readable representation of a message to an output stream.
    ...
    \param hovr_rsp the message to write.
    \return the output stream after the message has been written to it.
*/
inline
std::ostream& operator<<(std::ostream &os, const handover_rsp & hovr_rsp) {
    os << "handover_rsp[type: " << hovr_rsp.get_type();
    os << ",response_header=";
    os << hovr_rsp.response_header;
    os << "]";
    return os;
}
