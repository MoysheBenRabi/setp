#ifndef MXP_MESSAGE_HANDOVER_EVENT
#define MXP_MESSAGE_HANDOVER_EVENT
...
/*! A Handover event message, as specified by MXP.
 */
class handover_event : public message {
public:
    ...
    /*! Description of the handed over object */
    object_fragment object_header;    // X
    /*! Constructor. */
    handover_event() : message(HANDOVER_EVENT) {}

    /*! Virtual destructor. */
    virtual ~handover_event() {}

    /*! Copy constructor.

        \param hovr_event the object to base this copy on.
    */
    handover_event(const handover_event & hovr_event) : message(HANDOVER_EVENT),
        source_bubble_id(hovr_event.source_bubble_id),
        target_bubble_id(hovr_event.target_bubble_id),
        object_header(hovr_event.object_header) {
    }

    /*! Assignment operator.

        \param hovr_event the object to base this object on.
        \return a reference to this object, after the assignemt has been made.
    */
    handover_event& operator=(const handover_event & hovr_event) {
        if (this == &hovr_event) {
            return *this;
        }
        source_bubble_id = hovr_event.source_bubble_id;
    ...
    virtual bool equals(const message & other) const {
        return (this == &other) ||
               ((get_type() == other.get_type()) &&
                (source_bubble_id == ((handover_event &)other).source_bubble_id) &&
                (target_bubble_id == ((handover_event &)other).target_bubble_id) &&
                (object_header == ((handover_event &)other).object_header));
    }
...
/*! Write a human-readable representation of a message to an output stream.
    ...
    \param hovr_event the message to write.
    \return the output stream after the message has been written to it.
*/
inline
std::ostream& operator<<(std::ostream &os, const handover_event & hovr_event) {
    os << "handover_event[type: " << hovr_event.get_type();
    os << ",source_bubble=";
    os << hovr_event.source_bubble_id;
    os << ",target_bubble=";
    os << hovr_event.target_bubble_id;
    os << ",object_heder=";
    os << hovr_event.object_header;
    os << "]";
